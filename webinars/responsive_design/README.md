...
現代では様々な画面サイズのデバイスが存在し、ユーザーはそれぞれのデバイスで快適にウェブサイトを閲覧するため、それぞれのデバイスに最適化された表示が求められます。

今回のウェビナーではレスポンシブデザインの基本的なところから、実践的なCSSのテクニックまでを解説します。
普段の開発からレスポンシブなデザインを意識して、UI崩れが起きない画面を目指しましょう!
...
**レスポンシブデザイン**は2010年5月に登場しました。これはウェブデザイナーのイーサン・マーコットが提唱したもので、以下の3つの要素がレスポンシブデザインの核として紹介されました。
...
レスポンシブデザインは、WebサイトのCVR(コンバージョン率)やユーザー体験を上げるために非常に重要です。

※CVR(コンバージョン率) : 訪問者のうち、実際に商品購入や会員登録などのアクションを起こした割合のことです。
...
- **%（パーセント）**：親要素に対する割合でサイズを指定します。
  - 用途：コンテナ幅や画像サイズの割合指定
- **em**：要素のフォントサイズを基準に相対的にサイズを指定します。正直あまり使わないです。
- **rem**：ルート要素（`html`）のフォントサイズを基準に相対的にサイズを指定します。
  - 用途：余白・フォントサイズ指定
- **vw / vh**：ビューポート（画面）の幅／高さに対する割合で指定します。
  - 用途：画面サイズに応じた文字サイズ/高さの調整
- **vmin / vmax**：ビューポートの幅と高さのうち、小さい方／大きい方に対する割合で指定します。正直あまり使わないです。
...
- **minmax(min_value1, max_value2)**：指定した最小値と最大値の範囲で値を返します。正直あまり使わないです。
...
メディアクエリにおいて、どのサイズを指定するか=ブレイクポイントをどうするかは、プロジェクトやデザインによって異なります。
...
いくつか実際にレスポンシブ対応を行う際のポイントを紹介します。
